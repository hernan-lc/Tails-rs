...
pub(super) fn native_dns_resolve(
    ...
    match rrtype.to_uppercase().as_str() {
        "A" => native_dns_resolve4(interp, _this, &[Value::from_string(domain)]),
        "AAAA" => native_dns_resolve6(interp, _this, &[Value::from_string(domain)]),
        "MX" => native_dns_resolve_mx(interp, _this, &[Value::from_string(domain)]),
        _ => {
    ...
    let result_idx = interp.gc.allocate(
        ...
        HeapValue::Object(JsObject {
            properties: crate::props! {
                "address" => Value::from_string(hostname.clone()),
