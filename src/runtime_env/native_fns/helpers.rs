...
/// Trait for convenient argument extraction from native function parameter slices.
///
/// Provides helper methods to extract common argument types with defaults,
/// reducing repetitive `args.first().map(...).unwrap_or(...)` patterns.
///
/// # Example
///
/// ```
/// let n = args.first_f64(0.0);
/// let port = args.get_f64(1, 3000.0) as u16;
/// let flag = args.first_bool(false);
/// let name = args.require_string(0)?;
/// ```
pub(super) trait ArgExtractor {
    /// Extract the first argument as f64, with a default value.
    fn first_f64(&self, default: f64) -> f64;
    
    /// Extract argument at index as f64, with a default value.
    fn get_f64(&self, index: usize, default: f64) -> f64;
    
    /// Extract the first argument as i64, with a default value.
    fn first_i64(&self, default: i64) -> i64;
    
    /// Extract the first argument as boolean, with a default value.
    fn first_bool(&self, default: bool) -> bool;
    
    /// Extract the first argument as Option<String>.
    fn first_string(&self) -> Option<String>;
    
    /// Extract argument at index as String, returning error if not found or wrong type.
    fn require_string(&self, index: usize) -> crate::errors::Result<String>;
    
    /// Extract the first argument as Option<i64>.
...
impl ArgExtractor for &[Value] {
    fn first_f64(&self, default: f64) -> f64 {
        self.first().map(to_f64).unwrap_or(default)
    }
    
    fn get_f64(&self, index: usize, default: f64) -> f64 {
        self.get(index).map(to_f64).unwrap_or(default)
    }
    
    fn first_i64(&self, default: i64) -> i64 {
        self.first().map(to_i64).unwrap_or(default)
    }
    
    fn first_bool(&self, default: bool) -> bool {
        self.first().map(is_truthy).unwrap_or(default)
    }
    
    fn first_string(&self) -> Option<String> {
        self.first().and_then(|v| match v {
            Value::String(s) => Some(s.clone()),
            _ => None,
        })
    }
    ...
    fn require_string(&self, index: usize) -> crate::errors::Result<String> {
        self.get(index)
            ...
            .ok_or_else(|| crate::errors::Error::TypeError(
                format!("Expected string argument at index {}", index)
            ))
    }
