...
pub fn type_err(
    ...
    issue_custom(
        "invalid_type",
        custom_error.unwrap_or(&format!("Expected {}", expected)),
        expected,
        &type_name(value),
    )
}
    ...
    issues
        ...
        .map(|issue| {
            ...
            if let (Some(exp), Some(rec)) = (&issue.expected, &issue.received) {
                if !exp.is_empty() && !rec.is_empty() {
                    line.push_str(&format!(" (expected: {}, received: {})", exp, rec));
                }
            }
            line
        })
