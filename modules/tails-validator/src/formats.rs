pub fn is_valid_email(s: &str) -> bool {
    ...
    if domain_parts.iter().any(|p| p.is_empty()) { return false; }
    local.chars().all(|c| c.is_alphanumeric() || c == '.' || c == '_' || c == '+' || c == '-')
        && domain.chars().all(|c| c.is_alphanumeric() || c == '.' || c == '-')
}

pub fn is_valid_url(s: &str) -> bool { s.starts_with("http://") || s.starts_with("https://") }

pub fn is_valid_uuid(s: &str) -> bool {
    let parts: Vec<&str> = s.split('-').collect();
    if parts.len() != 5 { return false; }
    [8, 4, 4, 4, 12].iter().zip(parts.iter()).all(|(&len, &part)| part.len() == len && part.chars().all(|c| c.is_ascii_hexdigit()))
}
...
pub fn is_valid_ipv6(s: &str) -> bool {
    let parts: Vec<&str> = s.split(':').collect();
    if parts.len() < 3 || parts.len() > 8 { return false; }
...
pub fn is_valid_base64(s: &str) -> bool {
    ...
    s.chars().all(|c| c.is_ascii_alphanumeric() || c == '+' || c == '/' || c == '=') && s.len() % 4 == 0
}
